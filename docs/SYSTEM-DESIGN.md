...
flowchart TB
    subgraph IN["Точки входа, пакет internal/api"]
        ...
        P5["GET /healthz, /readyz, /metrics<br/>служебные"]
    end
...
Три последние строки это цена отказа от нейросетевой модели: заглавная буква
для правил такой же признак, как окончание слова, и снять её требование
целиком нельзя, иначе фамилией становится любое слово вроде «клиентов» или
«слева». Поэтому строчная фамилия разбирается отдельным правилом по опоре в
соседних словах. Это единственное место, где требование задания выполняется не
полностью, порог и список опор дорабатываются, а числа качества в
[BENCHMARKS.md](../BENCHMARKS.md) сняты до этой доводки.
