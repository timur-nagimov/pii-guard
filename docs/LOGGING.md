...
| ответ без маскирования при внутренней ошибке в режиме `on_error: open` | `mask` | `degraded` |

Последние два добавлены сверх очевидного. Отказанная попытка развернуть маску
для службы контроля интереснее удавшейся: видно, кто просил чужое. А ответ без
маскирования означает, что персональные данные ушли назад открытым текстом, и
для отчётности это событие важнее удачной работы.
