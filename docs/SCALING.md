...
**Числа модели сняты до ночного прогона улучшений и теперь консервативны.**
Стоимость обработки с тех пор изменилась дважды: переиспользование документа
её снизило, устойчивость к шуму подняла, починка приведения похожих букв снова
снизила. На ноутбуке разработчика чистый разбор текста стоил 0.41 мс на
килобайт, сейчас даёт 0.34.

Прямо пересчитывать модель по этому числу нельзя: её коэффициенты сняты на
стенде и включают сеть, разбор JSON и запись в хранилище, а ноутбучный замер
меряет только обработку текста. Соотношение машин известно, стенд втрое
медленнее, но проверять его надо заново после выката, а не переносить старое.

Практическое следствие: модель сейчас завышает стоимость примерно на пятую
часть, то есть ошибается в безопасную сторону и даёт лишний запас. Пересчитать
её стоит после выката новой сборки на стенд, прогоном потолка ступенями.
