...
Запуск: python3 scripts/loadtest_ids.py исходный-набор новый-набор метка
"""
...
def main(argv):
    """Проставляет метку прогона идентификаторам набора.

    Набор переписывается построчно, а не собирается в памяти: файлы нагрузки
    доходят до сотен тысяч записей, и держать их целиком незачем.
    """
    if len(argv) != 4:
