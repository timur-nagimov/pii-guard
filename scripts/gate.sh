...
for arg in "$@"; do
  case "$arg" in
    ...
    --list)
      ...
      echo "  12  новых файлов вне git нет"
      echo "  13  ссылки в документах целы"
      echo "  14  схемы Mermaid разбираются"
      echo "  15  запрещённые зависимости не добавлены"
      exit 0
      ;;
  esac
done
...
# Файл, не заведённый в git, в архив не попадёт. Сегодня это едва не стоило
# сдачи: пакет журналирования из девятнадцати файлов, четыре документа и все
# скрипты ворот лежали незаведёнными, и архив просто не собирался. Проверка
# стоит доли секунды и закрывает целый класс беды.
UNTRACKED="$(git ls-files --others --exclude-standard 2>/dev/null | grep -vE '^(corpus|dist)(/|$)' || true)"
if [ -z "$UNTRACKED" ]; then
  ok "новых файлов вне git нет"
else
  bad "файлы не заведены в git и в архив не попадут:"
