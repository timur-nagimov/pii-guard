...
if logfile and pathlib.Path(logfile).exists():
    channels["журнал"] = pathlib.Path(logfile)
else:
    print("ВНИМАНИЕ: файла журнала нет, проверены только показатели.")
    print("          Ворота не могут считать эту проверку полной.")
