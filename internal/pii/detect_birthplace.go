...
func birthContinues(d *Doc, ws []addrWord, j, start int) bool {
	...
	if strings.Contains(gap, ",") && !ws[j].title && !addrIsTypeWord(ws[j].lower) {
		...
		// Россия». В нижнем регистре слово не заглавное, и без этого правила
		// запятая обрывала бы фрагмент на названии города.
		if !birthPlaceContinuation(ws, j) && !birthRegionTail(ws, j) &&
			!birthCountryWords[ws[j].lower] {
			return false
		}
	}
	return true
...
// токенизатором на части, поэтому смотрим на несколько слов вперёд. Сравнение
// идёт по приведённому к нижнему регистру слову: в словаре регионов ключи
// только строчные.
...
// автономный округ». Подмена кириллицы похожей латиницей снимается свёрткой
// омоглифов: «кaлугa» с латинской «a» опознаётся так же, как «калуга».
