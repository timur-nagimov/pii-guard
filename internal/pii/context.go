...
var ctxPIIAnchors = []string{
	...
	"cardholder", "внж", "рвп", "загранпаспорт", "к/п", "код подр", "код п/п",
}
...
var ctxRoleMarkers = []string{
	...
	// Вопросы к модели и должности: по ним упоминание современника
	// опознаётся как публичное лицо, а не как клиент.
	"кто такой", "кто такая", "расскажи о", "расскажи про", "биография",
	"чем известен", "что сделал", "что думал", "губернатор", "мэр", "глава",
	"председател", "депутат", "предпринимател", "основател", "блогер",
	"телеведущ", "миллиардер", "политик", "бизнесмен",
}
...
var ctxPersonalAnchors = []string{
	...
	"клиент", "заявител", "заказчик", "получател", "адресат", "плательщик",
