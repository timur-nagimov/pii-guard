...
func (s *Server) handleChatCompletions(w http.ResponseWriter, r *http.Request) {
	...
	s.proxyUpstream(w, r, sys, proxyCall{
		outBody: outBody, back: back, counts: counts, bodyLen: bodyLen, begin: begin,
	})
}
...
func (s *Server) prepareMaskedBody(w http.ResponseWriter, r *http.Request, sys config.System, cfg *config.Config, req chatRequest, raw map[string]any) ([]byte, map[string]string, map[string]int, string, error) {
	...
	// Карты заводит подготовка тела, а не само маскирование: они живут
	// дольше него. Обратное соответствие нужно уже на ответе модели, а число
	// найденных фрагментов уходит в аудит на каждом исходе, включая тот, где
	// модель не ответила вовсе.
...
// proxyCall — всё, что пересылке нужно после маскирования: готовое тело
// запроса, обратное соответствие плейсхолдеров для ответа и данные аудита.
// Отдельная структура вместо восьми параметров подряд (S107): соседние
// значения одного типа легко переставить местами при вызове.
...
func (s *Server) proxyUpstream(w http.ResponseWriter, r *http.Request, sys config.System, call proxyCall) {
	started := time.Now()
	resp, err := s.callUpstream(r, sys, call.outBody)
	if err != nil {
		...
		s.auditProcess(r, sys, proxyEvent(call.bodyLen, call.counts, call.begin), "error")
		return
	}
	...
	restored := restoreResponse(respBody, call.back)

	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	w.Header().Set("X-PII-Masked", itoa(len(call.back)))
	w.WriteHeader(resp.StatusCode)
	if _, err := w.Write(restored); err != nil {
		s.log.WarnContext(r.Context(), "не удалось записать ответ",
			logging.Event(logging.EventProxy), logging.Component("proxy"), logging.Err(err))
	}
	s.auditProcess(r, sys, proxyEvent(call.bodyLen, call.counts, call.begin), "ok")
}
...
func (s *Server) callUpstream(r *http.Request, sys config.System, body []byte) (*http.Response, error) {
	...
	// Подменить адрес снаружи нельзя: он берётся из настроек сервиса, а не
	// из запроса. Проверка закрывает другой случай — опечатку оператора:
	// адрес вида file:// или unix:// превратился бы в обращение к локальному
	// ресурсу, а говорить с моделью можно только по http и https.
